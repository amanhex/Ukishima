...
- **Wallpaper system** — `awww` backend with a shuffled bag, per-monitor assignment, animated transitions, live wallpapers (`mpvpaper`), a per-wallpaper fit control (Cover / Contain / Stretch / Center) that rescales the screen in place, and a live palette that retints the whole UI plus the terminal on every change.
- **Wallhaven online search** — browse wallhaven from the wallpaper strip: a toggled chip loads the hot feed, type-to-search refines it, paging chevrons and arrow keys scroll through chunks, and Enter applies. A sort dropdown switches between Hot, Latest, Top, Random and Top Liked, and the all/still/live local filter swaps out for the wallhaven sort while browsing. Thumbnails stream straight from the API into memory — nothing is cached to disk, and each page's thumbs are released on page/close.
